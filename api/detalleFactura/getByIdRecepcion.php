...
$cant_und = 0;
$cant_caja = 0;
for ($i = 0; $i < count($detalleFactura_arr["data"]); $i++) {
    if ($detalleFactura_arr["data"][$i]['df_nombre_und_detfac'] == 'CAJA') {
        $cantidad = $detalleFactura_arr["data"][$i]['df_cantidad_detfac'] * 1;
        $cant_caja = $cantidad + $cant_caja;
    } else if ($detalleFactura_arr["data"][$i]['df_nombre_und_detfac'] == 'UND') {
        $cantidad = $detalleFactura_arr["data"][$i]['df_cantidad_detfac'] * 1;
        $cant_und = $cant_und + $cantidad;
    }
}

for ($i = 0; $i < count($detalleFactura_arr["data"]); $i++) {
    $detalleFactura_arr["data"][$i]['df_cantidad_caja'] = $cant_caja;    
    $detalleFactura_arr["data"][$i]['df_cantidad_und'] = $cant_und;    
}
